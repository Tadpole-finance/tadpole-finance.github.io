...
change_environment = function(chainId){
	...
	if(chainId!='0x38'){
		$('#not-bsc-alert').removeClass('d-none');
...
var go_depo = async function(id){
	...
	if(cont.id=='bnb'){
		...
		cToken =  new web3.eth.Contract(cEtherAbi, cont.address);
...
var addTenToMetamask = async function(){
	...
	params: {
	  ...
	  options: {
		...
		decimals: 18, // The number of decimals in the token
		image: 'http://app.tadpole.finance/assets/images/tokens/ten_32.png', // A string url of the token logo
	  },
	},
	});
}
